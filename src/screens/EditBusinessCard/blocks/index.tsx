...
const SelectBusinessCardBlock: React.FC<IProps> = ({ navigation }) => {
  ...
  const { fetch: updateSpecialist, status: statusSpecialist } = useUpdateSpecialist();
  const { businessCards } = useSelector((s: RootState) => s?.myBusinessCard);
  const { setBackgroundColorCard } = clientActionCreators();
  ...
  React.useEffect(() => {
    setBackgroundColorCard(cards[index]);
  }, [index, cards]);

  React.useEffect(() => {
    if (statusSpecialist === APIStatus.Success) navigation.goBack();
  }, [statusSpecialist]);

  const handleOnPress = React.useCallback(() => {
    if (cards) {
      updateSpecialist(myProfile);
    }
  }, [cards, myProfile]);
  ...
  React.useEffect(() => {
    !businessCards?.data && fetchBusinessCards();
  }, []);
  ...
  return (
    <View
      ...
      <WrapperAsyncRequest status={status}>
        ...
        <CustomButton onPress={handleOnPress} label={'Сохранить'} type="primary" status={statusSpecialist} />
      </WrapperAsyncRequest>
    </View>
  );
};
