...
import { Text, KeyboardAvoidingView, Platform, View, TouchableOpacity, Keyboard } from 'react-native';

import { NavigationType } from '../../navigation/MainStackNavigator';
import { MainLayouts } from '../../layouts';
import { APIStatus } from '../../lib/axiosAPI';
import { CodeField, CroppedModalWindow, LoaderModal } from '../../components';
...
interface IProps {
  ...
  route: RouteProp<{ params: { goBack: boolean } }, 'params'>;
}

const EnterPinCode: React.FC<IProps> = ({ navigation, route }) => {
  const { fetch: setPin, status } = useSetPin();
  const [value, setValue] = React.useState<string>('');
  const [repeatValue, setRepeatValue] = React.useState<string>('');
  const [isError, setIsError] = React.useState<boolean>(false);
  const [onVisibleLoader, setOnVisibleLoader] = React.useState<boolean>(false);
  const [isVisibleInfoModal, setIsVisibleInfoModal] = React.useState<boolean>(false);

  const handleOnMiss = React.useCallback(() => {
  ...
  React.useEffect(() => {
    if (status === APIStatus.Success) {
      setOnVisibleLoader(false);
      if (!isUndefined(route.params)) navigation.goBack();
      else setIsVisibleInfoModal(true);
    }
    if (status === APIStatus.Loading) {
      Keyboard.dismiss();
      setOnVisibleLoader(true);
    }
  }, [status]);
    ...
    <MainLayouts customContainerStyle={{ marginTop: 32 }}>
      ...
      <KeyboardAvoidingView
        ...
        <Svg width="64" height="64" fill="none" viewBox="0 0 64 64">
          <Path
            fill="#F1FAFD"
            d="M32.015 19.75c-11.875 0-21.5 9.626-21.5 21.5s9.625 21.5 21.5 21.5c11.874 0 21.5-9.626 21.5-21.5s-9.626-21.5-21.5-21.5z"
          />
          <Path
            fill="#fff"
        ...
        />
        {isUndefined(route.params) && (
          <TouchableOpacity activeOpacity={0.8} onPress={handleOnMiss}>
            <Text
              style={{
